(function($) {
    $(document).ready(function() {
        ...
        $('#pk-chat-rp-btn').on('click', function(e) {
            e.stopPropagation(); $('#pk-chat-emoji-picker').hide(); rpModal.fadeToggle(150);
        });
        ...
        $('#pk-rp-submit').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if(res.status === 'success') { rpModal.fadeOut(150); $('#pk-rp-amount').val(''); $('#pk-rp-count').val(''); fetchMessages(true); } else { alert(res.msg); }
                }
            });
        });
        $(document).on('click', '.pk-redpacket-box', function() {
            $.ajax({
                ...
                success: function(res) { if(res.status === 'success') { alert('🎉 ยินดีด้วย! คุณได้รับอั่งเปาจำนวน ' + res.amount + ' เครดิต'); } else { alert(res.msg); } }
            });
        });
        ...
        $(document).on('click', function(e) {
            if (!$(e.target).closest('#pk-chat-emoji-picker, #pk-chat-emoji-btn, #pk-chat-rp-modal, #pk-chat-rp-btn').length) { emojiPicker.fadeOut(150); rpModal.fadeOut(150); }
        });
        ...
        $('#pk-chat-file-upload').on('change', function() {
            ...
            var inputField = $('#pk-chat-input'); inputField.val('กำลังอัปโหลดรูปภาพ...').prop('disabled', true);
            $.ajax({
                url: apiUrl + '&action=upload', type: 'POST', data: formData, processData: false, contentType: false, dataType: 'json',
                success: function(res) {
                    inputField.val('').prop('disabled', false).focus();
                    if(res.status === 'success') {
                        $.ajax({ url: apiUrl + '&action=send&room_id=' + currentRoomId, type: 'GET', data: { message: '[img]' + res.url + '[/img]' }, dataType: 'json', success: function(s) { if(s.status === 'success') fetchMessages(true); } });
                    } else { alert(res.msg); }
        ...
        $(document).on('click', '.pk-msg-reply', function() {
            var user = $(this).data('user'); var input = $('#pk-chat-input'); var currentVal = input.val();
            if(currentVal.indexOf('@' + user) === -1) input.val('@' + user + ' ' + currentVal).focus(); else input.focus();
        });
        
        // --- 💖 ระบบคลิกรีแอคชั่น ---
        $(document).on('click', '.pk-msg-add-react, .pk-react-pill', function() {
            var msgId = $(this).data('id');
            var emoji = $(this).data('emoji');
            $.ajax({
                url: apiUrl + '&action=react&msg_id=' + msgId + '&reaction=' + encodeURIComponent(emoji),
                type: 'GET',
                dataType: 'json',
                success: function(res) { if(res.status === 'success') fetchMessages(false); } // ไม่ต้องเลื่อนจอ
            });
        });
        ...
        $('.pk-room-tab').on('click', function() {
            ...
            chatBox.html('<div style="text-align:center; padding:15px; color:#999;">กำลังโหลดข้อความ...</div>'); fetchMessages(true); 
        });

        function toggleChat() {
            chatBody.slideToggle(200, function() {
                var isHidden = chatBody.is(':hidden'); toggleBtn.text(isHidden ? '☐' : '_'); localStorage.setItem('prasopkan_chat_state', isHidden ? 'closed' : 'open');
            });
        }
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.status === 'success') {
                        ...
                        var isReactionEnabled = (res.enable_reaction == 1); // รับค่าตั้งค่า
                        ...
                        if(res.data.length === 0) {
                            ...
                            $.each(res.data, function(index, item) {
                                ...
                                // 💖 สร้างปุ่มให้กดรีแอคชั่นท้ายข้อความ
                                var reactBtn = isReactionEnabled ? '<span class="pk-msg-add-react" data-id="'+item.msg_id+'" data-emoji="👍" title="ถูกใจ">👍</span><span class="pk-msg-add-react" data-id="'+item.msg_id+'" data-emoji="❤️" title="รักเลย">❤️</span><span class="pk-msg-add-react" data-id="'+item.msg_id+'" data-emoji="😂" title="ฮ่าๆ">😂</span>' : '';
                                
                                // 💖 แสดงยอดรวมรีแอคชั่นที่ถูกกดแล้ว
                                var pills = '';
                                if(item.reactions) {
                                    $.each(item.reactions, function(emoji, data) {
                                        var activeClass = data.me ? ' active' : ''; // ถ้าเราเคยกดให้เป็นสีฟ้า
                                        pills += '<span class="pk-react-pill'+activeClass+'" data-id="'+item.msg_id+'" data-emoji="'+emoji+'">'+emoji+' '+data.count+'</span>';
                                    });
                                    if(pills !== '') pills = '<div class="pk-reactions-display">' + pills + '</div>';
                                }

                                html += '<div class="pk-msg-item"><b style="color:' + item.color + ';">' + item.username + ':</b> ' + item.message + '<span class="pk-msg-time">[' + item.time + ']</span>' + replyBtn + delBtn + reactBtn + pills + '</div>';
                            });
                        }
        ...
        $(document).on('click', '.pk-msg-delete', function() {
            if(confirm('คุณต้องการลบข้อความนี้ใช่หรือไม่?')) {
                $.ajax({ url: apiUrl + '&action=delete&msg_id=' + $(this).data('id'), type: 'GET', dataType: 'json', success: function(res) { if(res.status === 'success') fetchMessages(false); else alert(res.msg); } });
            }
        });

        function sendMessage(e) {
            if(e) e.preventDefault();
            var inputField = $('#pk-chat-input'); if(inputField.length === 0) return; 
            var msg = inputField.val();
            if(msg.trim() !== '') {
                inputField.prop('disabled', true); emojiPicker.fadeOut(150); 
                $.ajax({
                    url: apiUrl + '&action=send&room_id=' + currentRoomId, type: 'GET', data: { message: msg }, dataType: 'json',
                    success: function(res) { inputField.prop('disabled', false).val('').focus(); if(res.status === 'success') fetchMessages(true); else alert(res.msg); },
