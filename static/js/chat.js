(function($) {
    $(document).ready(function() {
        ...
        // --- 🪄 สร้างหน้าต่างเมนูกดค้าง (Messenger Style) ---
        ...
        function renderShopItems(type) {
            ...
            if(type !== 'gacha' && eq && eq !== '') { html += '<div class="pk-shop-item" style="grid-column: span 2; background: rgba(255,0,0,0.1); border-color: red;"><button class="pk-btn-equip" style="background: red;" data-key="" data-type="'+type+'">ถอดไอเทมที่ใส่อยู่ทิ้ง</button></div>'; }
        ...
        $(document).on('click', '.pk-btn-buy', function() {
            ...
            if(confirm(confirmMsg)) {
                btn.text('กำลังซื้อ...').prop('disabled', true);
                $.ajax({
                    url: apiUrl + '&action=shop_buy&item_key='+key+'&item_type='+type, type: 'GET', dataType: 'json',
                    success: function(res) { 
                        if(res.status === 'success') { if(res.is_gacha) { alert('🎉 ยินดีด้วย! คุณเปิดกล่องสุ่มได้รับ:\n\n✨ [' + res.won_name + '] ✨\n\nไปที่แท็บอื่นเพื่อกดสวมใส่ได้เลย!'); } else { alert('ซื้อสำเร็จ!'); } loadShop(); } 
                        else { alert(res.msg); loadShop(); } 
                    }
                });
            }
        });
        $(document).on('click', '.pk-btn-equip', function() { $.ajax({ url: apiUrl + '&action=shop_equip&item_key='+$(this).data('key')+'&item_type='+$(this).data('type'), type: 'GET', dataType: 'json', success: function(res) { if(res.status === 'success') { loadShop(); fetchMessages(false); } else { alert(res.msg); } } }); });
        ...
        // --- 🗑️ นำปุ่มที่รกๆ ออกไปจากการเรนเดอร์ ---
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.status === 'success') {
                        // เก็บค่าสิทธิ์ผู้ใช้ไว้ใช้กับระบบกดค้าง
                        window.pkIsAdmin = res.is_admin; 
                        window.pkCurrentUid = res.current_uid;
                        window.pkEnableMention = res.enable_mention;
                        window.pkEnableReaction = res.enable_reaction;
                        ...
                        if(res.data.length === 0) { html = '<div style="text-align:center; padding:15px; color:var(--pk-text-muted);">ยังไม่มีคนคุย มาเริ่มคุยกันเลย!</div>'; } else {
                            $.each(res.data, function(index, item) {
                                ...
                                // ฝังข้อมูลไว้ที่กล่องสำหรับให้ระบบกดค้างดึงไปใช้
                                ...
                                html += '<div class="pk-bubble"' + bubbleStyle + bubbleData + '>' + item.message + '</div>';
        ...
        // --- ⏳ ตัวจับเวลากดค้าง (Long Press Events) แก้ไข Ghost Click ---
        var pressTimer;
        var isLongPress = false; // ตัวแปรเช็คว่าเป็นการกดค้างหรือไม่

        // ปิดเมนูคลิกขวาเดิมของเบราว์เซอร์
        ...
        $(document).on('touchstart mousedown', '.pk-bubble', function(e) {
            var $bubble = $(this);
            if($bubble.data('bot') == '1') return; // บอทกดค้างไม่ได้
            
            isLongPress = false; // รีเซ็ตค่าทุกครั้งที่เริ่มแตะ
            
            pressTimer = setTimeout(function() {
                isLongPress = true; // ยืนยันว่าเป็นการกดค้างแล้ว
                
                // 📱 เพิ่มลูกเล่น: สั่นเบาๆ 50ms ให้รู้ว่ากดติดแล้ว (รองรับ Android)
                if (navigator.vibrate) { navigator.vibrate(50); }

                var msgId = $bubble.data('id');
                var user = $bubble.data('user');
                ...
                if(window.pkIsAdmin || window.pkCurrentUid == 1) {
                    $('#pk-menu-delete').attr('data-id', msgId).show();
                } else {
                    $('#pk-menu-delete').hide();
                }
        ...
        // ถ้านิ้วขยับ (ไถหน้าจอ) ให้ยกเลิกการกดค้างทันที
        $(document).on('touchmove mousemove', '.pk-bubble', function() {
            clearTimeout(pressTimer);
            isLongPress = false;
        });

        // เมื่อปล่อยนิ้ว
        $(document).on('touchend mouseup', '.pk-bubble', function(e) {
            clearTimeout(pressTimer);
            // 🛡️ ป้องกัน Ghost Click: ถ้าเพิ่งกดค้างสำเร็จ ห้ามเบราว์เซอร์ส่งคำสั่ง 'click' ไปทำอย่างอื่นต่อ
            if(isLongPress) {
                e.preventDefault();
                e.stopPropagation();
            }
        ...
        // ปิดหน้าต่างเมนูเมื่อกดพื้นที่ว่างสีดำ
        $('#pk-chat-box').on('click', '#pk-chat-action-overlay', function(e) {
            if(e.target.id === 'pk-chat-action-overlay') { $(this).fadeOut(150); }
        });

        // 🔘 กดสั่งการจากเมนู
        $(document).on('click', '.pk-msg-menu-react', function() {
            var msgId = $(this).attr('data-id'); var emoji = $(this).attr('data-emoji');
            $('#pk-chat-action-overlay').fadeOut(150);
            $.ajax({ url: apiUrl + '&action=react&msg_id=' + msgId + '&reaction=' + encodeURIComponent(emoji), type: 'GET', dataType: 'json', success: function(res) { if(res.status === 'success') fetchMessages(false); } });
        });

        $(document).on('click', '#pk-menu-reply', function() {
            var user = $(this).attr('data-user');
            $('#pk-chat-action-overlay').fadeOut(150);
            var input = $('#pk-chat-input'); var currentVal = input.val();
            if(currentVal.indexOf('@' + user) === -1) input.val('@' + user + ' ' + currentVal).focus(); else input.focus();
        });

        $(document).on('click', '#pk-menu-delete', function() {
            var msgId = $(this).attr('data-id');
            $('#pk-chat-action-overlay').fadeOut(150);
            if(confirm('ต้องการลบข้อความนี้?')) {
                $.ajax({ url: apiUrl + '&action=delete&msg_id=' + msgId, type: 'GET', dataType: 'json', success: function(res) { if(res.status === 'success') fetchMessages(false); else alert(res.msg); } });
            }
        });
